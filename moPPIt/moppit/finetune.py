...
class CosineAnnealingWithWarmup(_LRScheduler):
    ...
    # def get_lr(self):
    #     if self.last_epoch < 114:
    #         # Linear warmup phase from base_lr to max_lr
    #         return [self.base_lr + (self.max_lr - self.base_lr) * (self.last_epoch / self.warmup_steps) for base_lr in self.base_lrs]
    #     # elif self.warmup_steps <= self.last_epoch < 152:
    #     #     # Cosine annealing phase from max_lr to min_lr
    #     #     progress = (self.last_epoch - self.warmup_steps) / (self.total_steps - self.warmup_steps)
    #     #     cosine_decay = 0.5 * (1 + np.cos(np.pi * progress))
    #     #     decayed_lr = self.min_lr + (self.max_lr - self.min_lr) * cosine_decay
...
def main():
    ...
    train_dataset = load_from_disk('/home/tc415/muPPIt/dataset/train/correct_pepnn_biolip_train')
    val_dataset = load_from_disk('/home/tc415/muPPIt/dataset/val/correct_pepnn_biolip_val')
    tokenizer = AutoTokenizer.from_pretrained("facebook/esm2_t33_650M_UR50D")
    ...
    logger = WandbLogger(project=f"bind_evaluator",
                         name=f"finetune_lr={args.lr}_dropout={args.dropout}_nlayers={args.n_layers}_dmodel={args.d_model}_nhead={args.n_head}_dinner={args.d_inner}",
                         # display on the web
                         # save_dir=f'./pl_logs/',
