...
def main(args):
    ...
    print(
        f"Generation: -1\tBinder: {binders[0].binder_seq}\tScore: {binders[0].score-args.length}\tPPL: {binders[0].ppl}")
    ...
    while no_improvement_generations < max_tolerance:
        # binders = sorted(binders, key=lambda binder: (binder.score, binder.ppl))
        previous_score = binders[0].score - args.length
        previous_ppl = binders[0].ppl

        if binders[0].score <= args.length:
            break
        ...
        new_binders = sorted(new_binders, key=lambda binder: binder.score - args.length)

        score = new_binders[0].score - args.length

        if score < previous_score or new_binders[0].ppl < previous_ppl:
            no_improvement_generations = 0
            print(
                f"Generation: {generation}\tBinder: {new_binders[0].binder_seq}\tScore: {score}\tPPL: {new_binders[0].ppl}")
        else:
    ...
    print(f"Generation: {generation}\tBinder: {binders[0].binder_seq}\tScore: {binders[0].score-args.length}\tPPL: {binders[0].ppl}")
    ...
    while no_improvement_generations < max_tolerance:
        ...
        if score <= 0 and new_binders[0].ppl < previous_ppl:
            no_improvement_generations = 0
            print(
                f"Generation: {generation}\tBinder: {new_binders[0].binder_seq}\tScore: {score}\tPPL: {new_binders[0].ppl}")
        else:
