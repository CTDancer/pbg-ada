...
def main():
    ...
    parser.add_argument("-sm", default='/home/tc415/muPPIt/muppit/model_path/finetune_bindevaluator_0/model-epoch=30-val_mcc=0.60-val_loss=0.51.ckpt',
    ...
    with open('/home/tc415/muPPIt/dataset/biolip/biolip_correct.csv', 'r') as f:
        lines = f.readlines()
    ...
    for i in tqdm(range(len(lines))):
        ...
        binder = items[0]
        target = items[1]
        motif = line.split(',"')[1][:-1]
        # print(motif)

        prediction, _ = calculate_score(target, binder, model, args)
        # print(model.classification_threshold)

        binding_site = []
        for j in range(len(prediction)):
            if prediction[j] >= 0.5:  # 0.7331371903419495 # 0.6908
                binding_site.append(j)

        # print("Prediction: ", binding_site)

        L = len(target)
        gt = parse_motif(motif)

        acc, f1, mcc = compute_metrics(gt, binding_site, L)
        if mcc >= 0.7:
            print(f"Accuracy={acc}\tF1={f1}\tMCC={mcc}")
            print(f"Binder: {binder}\tTarget: {target}\tMotif: {motif}\n")

        with open("biolip_bindevaluator.txt", "a") as f:
            f.write(f"{acc}\t{f1}\t{mcc}\n")
