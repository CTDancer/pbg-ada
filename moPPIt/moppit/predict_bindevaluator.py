...
def main():
    ...
    parser.add_argument("-target_motifs", type=str, default=None)
    args = parser.parse_args()

    device = torch.device("cuda:0" if torch.cuda.is_available() else "cpu")

    # df = pd.read_csv('/home/tc415/moPPIt/moppit/known_binders.csv')
    # pdb_ids = df['ID']
    # seqs = df['Sequence']
    # protein_seqs = []
    # peptide_seqs = []
    # for seq in seqs:
    #     protein_seq = seq.split(':')[0]
    #     peptide_seq = seq.split(':')[1].strip()
    #     protein_seqs.append(protein_seq)
    #     peptide_seqs.append(peptide_seq)

    # for l in range(len(pdb_ids)):
    #     print(f"############### Predicting for {pdb_ids[l]} ###############")
    #     args.target = protein_seqs[l]
    #     args.binder = peptide_seqs[l]
    ...
    for i in range(len(prediction)):
            if prediction[i] >= 0.5:
                binding_site.append(i+1)
    print(f"Binder {args.binder}: ")
    print("Prediction: ", binding_site)


    # binders = ['QASESFTVTFTLLTG', 'LSTFDALFTIFSASG', 'ANCISASTGESYTFS', 'AILVEVKLFAFAYPK', 'EDRPAEAEAFEDPPS', 'ELYYPFALATVQVDP', 'NEFIVAFDIEEIKVT', 'KIFTPADMFRFEHLR', 'IMPMPLYANSAPASA', 'RPSTDFPQATSLYSP', 'LYVAIASASITTSFC', 'MSCFALIVAAYSSAQ', 'GLVYDMIFASFPAEK', 'SLTSATSPSGLETEA', 'QGVLETCSVDASSST']
    # candidates = []

    # for binder in binders:
    #     prediction, _ = calculate_score(args.target, binder, model, args)
    ...
    #     print("Prediction: ", binding_site)
    #     # for pos in parse_motif(args.target_motifs):
    #         # if prediction[pos-1].item() > 0.8:
    #         #     candidates.append(binder)
    #         # print(f"Position {pos} prob: {prediction[pos-1].item()}")
    #     print("----------------------------------------")
    # print(candidates)
    # prediction, _ = calculate_score(args.target, args.binder, model, args)
    # binding_site = []
