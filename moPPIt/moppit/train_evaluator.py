...
class PeptideModel(pl.LightningModule):
    ...
    def configure_optimizers(self):
        ...
        schedulers = CosineAnnealingWithWarmup(optimizer, warmup_steps=450, total_steps=11540,
...
def main():
    ...
    train_dataset = load_from_disk('/home/tc415/muPPIt/dataset/train/correct_train_dataset_drop_500')
    val_dataset = load_from_disk('/home/tc415/muPPIt/dataset/val/correct_val_dataset_drop_500')
    tokenizer = AutoTokenizer.from_pretrained("facebook/esm2_t33_650M_UR50D")
    ...
    model = PeptideModel(args.n_layers, args.d_model, args.n_head, 64, 128, args.d_inner, dropout=args.dropout,
    ...
    logger = WandbLogger(project=f"bind_evaluator",
                         name=f"optimize_lr={args.lr}_nlayers={args.n_layers}_dmodel={args.d_model}_nhead={args.n_head}_dinner={args.d_inner}",
                         # display on the web
                         # save_dir=f'./pl_logs/',
