...
def load_ptm_dataset(df: pd.DataFrame, config: DatasetConfig) -> DatasetDict:
    ...
    # remove sequences shorter than 20 and longer than 1025
    ds = ds.filter(lambda x: len(x[config.sequence_column_name]) >= 20)
    ds = ds.filter(lambda x: len(x[config.sequence_column_name]) <= 1000)
