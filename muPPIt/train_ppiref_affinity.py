...
def collate_fn(batch):
    ...
    for b in batch:
        binders.append(torch.tensor(b['binder_input_ids']).squeeze(0))  # shape: 1*L1 -> L1
        mutants.append(torch.tensor(b['mut_input_ids']).squeeze(0))  # shape: 1*L2 -> L2
        wildtypes.append(torch.tensor(b['wt_input_ids']).squeeze(0))  # shape: 1*L3 -> L3

        mut_affinities.append(torch.tensor(int(b['mut_affinity']) + 28))
        wt_affinities.append(torch.tensor(int(b['wt_affinity']) + 28))
...
def main(args):
    ...
    train_dataset = load_from_disk('/home/tc415/muPPIt/dataset/train/train_ppiref_affinity')
    val_dataset = load_from_disk('/home/tc415/muPPIt/dataset/val/val_ppiref_affinity')
    # val_dataset = None
    tokenizer = AutoTokenizer.from_pretrained("facebook/esm2_t33_650M_UR50D")
