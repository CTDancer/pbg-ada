...
def collate_fn(batch):
    ...
    for b in batch:
        ...
        if b['delta'] == 0:
            signs.append(torch.tensor(1))
        else:
            signs.append(torch.tensor(b['delta'] / abs(b['delta']) * (-1)))
        deltas.append(abs(torch.tensor(b['delta']).round()))
...
class muPPIt(pl.LightningModule):
    ...
    def load_weights(self, checkpoint_path):
        ...
        # Modify the state_dict to remove 'mapping' weights
        ...
        # Reinitialize the weights of the mapping layer
        nn.init.kaiming_uniform_(self.delta_map.weight)
        self.delta_map.weight.data *= 0.001
        ...
        for name, param in self.named_parameters():
            if 'diff_layer' in name or 'mim_layers' in name or 'cross_graph_att_layers' in name or 'delta_map' in name:
    ...
    def forward(self, binder_tokens, wt_tokens, mut_tokens):
        ...
        sign = mut_pred.squeeze(-1) - wt_pred.squeeze(-1)

        delta_edges = torch.norm(mut_binder_edges - wt_binder_edges, p=2, dim=1).unsqueeze(-1)
        
        delta_pred = self.delta_map(delta_edges)
        
        return sign, delta_pred.squeeze(-1)
    ...
    def training_step(self, batch, batch_idx):
        ...
        sign = batch['sign'].to(self.device)

        sign_pred, delta_pred = self.forward(binder_tokens, wt_tokens, mut_tokens)

        # pdb.set_trace()

        delta_loss = self.criterion(delta_pred, delta)    # make sure it's the right loss function
        sign_loss = torch.mean(F.relu(1 - sign * sign_pred))

        loss = delta_loss + sign_loss 

        # pdb.set_trace()
        self.log('delta_loss', delta_loss.item(), on_step=True, on_epoch=True, prog_bar=True, logger=True, sync_dist=True)
        self.log('sign_loss', sign_loss.item(), on_step=True, on_epoch=True, prog_bar=True, logger=True, sync_dist=True)
    ...
    def validation_step(self, batch, batch_idx):
        ...
        # pdb.set_trace()
        accuracy = ((delta_pred.round() == delta.round()) & (sign_pred == sign)).sum().item() / delta.shape[0]

        # pdb.set_trace()
        self.log('delta_loss', delta_loss.item(), on_step=True, on_epoch=True, prog_bar=True, logger=True, sync_dist=True)
        self.log('sign_loss', sign_loss.item(), on_step=True, on_epoch=True, prog_bar=True, logger=True, sync_dist=True)
        self.log('val_loss', loss.item(), on_step=True, on_epoch=True, prog_bar=True, logger=True, sync_dist=True)
        self.log('val_acc', accuracy, on_step=True, on_epoch=True, prog_bar=True, logger=True, sync_dist=True)
    ...
    def configure_optimizers(self):
        ...
        schedulers = CosineAnnealingWithWarmup(optimizer, warmup_steps=321, total_steps=3210,
...
def main():
    ...
    train_dataset = load_from_disk('/home/tc415/muPPIt/dataset/train/skempi')
    val_dataset = load_from_disk('/home/tc415/muPPIt/dataset/val/skempi')
    # val_dataset = None
    tokenizer = AutoTokenizer.from_pretrained("facebook/esm2_t33_650M_UR50D")
    ...
    print(f"Loading checkpoint from {args.sm}")

    run_id = str(uuid.uuid4())

    logger = WandbLogger(project=f"muppit",
                        #  name="debug",
                         name=f"delta_lr={args.lr}_gradclip={args.grad_clip}_int_MAE",
