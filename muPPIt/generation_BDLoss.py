...
def main(args):
    ...
    for epoch in tqdm(range(args.num_updates)):
        ...
        print(f"Mutant: {mut_pred.squeeze(0).argmax().item()}\tWildtype: {wt_pred.squeeze(0).argmax().item()}")

        mut_pred = (mut_pred - mut_pred.mean()) / mut_pred.std()
        wt_pred = (wt_pred - wt_pred.mean()) / wt_pred.std()
