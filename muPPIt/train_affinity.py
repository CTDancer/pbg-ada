...
def collate_fn(batch):
    ...
    for b in batch:
        ...
        mut_affinities.append(torch.tensor(b['mut_affinity']))
        wt_affinities.append(torch.tensor(b['wt_affinity']))
...
class muPPIt(pl.LightningModule):
    ...
    def training_step(self, batch, batch_idx):
        ...
        mut_pred_affinity, wt_pred_affinity = self.forward(binder_tokens, wt_tokens, mut_tokens)

        # pdb.set_trace()

        mut_loss = self.criterion(mut_pred_affinity, mut_affinity)
        wt_loss = self.criterion(wt_pred_affinity, wt_affinity)
        loss = mut_loss + wt_loss

        # penalty = torch.zeros_like(mut_pred_affinity)

        # condition1 = (mut_pred_affinity > wt_pred_affinity) & (mut_affinity < wt_affinity)
        # penalty[condition1] = torch.abs(mut_pred_affinity[condition1] - wt_pred_affinity[condition1])

        # condition2 = (mut_pred_affinity < wt_pred_affinity) & (mut_affinity > wt_affinity)
        # penalty[condition2] = torch.abs(mut_pred_affinity[condition2] - wt_pred_affinity[condition2])

        # loss = mut_loss + wt_loss + penalty.sum()*2

        self.log('train_mut_loss', mut_loss.item(), on_step=True, on_epoch=True, prog_bar=True, logger=True, sync_dist=True)
        self.log('train_wt_loss', wt_loss.item(), on_step=True, on_epoch=True, prog_bar=True, logger=True, sync_dist=True)
    ...
    def validation_step(self, batch, batch_idx):
        ...
        mut_accuracy = (mut_pred_affinity.round() == mut_affinity).sum() / mut_affinity.shape[0]
        wt_accuracy = (wt_pred_affinity.round() == wt_affinity).sum() / wt_affinity.shape[0]
    ...
    def configure_optimizers(self):
        ...
        schedulers = CosineAnnealingWithWarmup(optimizer, warmup_steps=840, total_steps=8400,
    ...
    def load_weights(self, checkpoint_path):
        ...
        # Reinitialize the weights of the layers inside the delta_linear Sequential
        # for layer in self.affinity_linear:
        ...
        for name, param in self.named_parameters():
            param.requires_grad = False
            if 'affinity_linear' in name or 'cross_graph_att_layers' in name or 'mim_layers' in name or 'diff_layer' in name: # or 'cross_graph_att_layers' in name or 'mim_layers' in name or 'diff_layer' in name
