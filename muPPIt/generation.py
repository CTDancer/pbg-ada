...
def main(args):
    ...
    pepmlm_results = generate_peptide(args.wt, args.binder_length, args.top_k, args.num_binders)
    ...
    for _ in tqdm(range(args.num_updates)):
        ...
        delta = muppit(binder_node_representation, binder_edge_representation, wt_tokens, mut_tokens)

        loss = torch.relu(6 - delta)

        print(f"Delta Logits:{delta.item()}")
        ...
        with torch.no_grad():
            binder_node_representation -= args.step_size * binder_node_representation.grad
