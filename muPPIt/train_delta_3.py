...
class CustomDataModule(pl.LightningDataModule):
    ...
    def val_dataloader(self):
        ...
        return DataLoader(self.val_dataset, batch_size=1, collate_fn=collate_fn, num_workers=8, 
                          pin_memory=True)
...
class muPPIt(pl.LightningModule):
    ...
    def training_step(self, batch, batch_idx):
        ...
        delta_pred = self.forward(binder_tokens, wt_tokens, mut_tokens)

        # pdb.set_trace()

        loss = self.criterion(delta_pred, delta.float())
    ...
    def validation_step(self, batch, batch_idx):
        ...
        positive_mask = (delta == 1).int()
        positive_total = positive_mask.sum()
        if positive_total > 0:
            positive_accuracy = ((delta_pred == delta).int() * positive_mask).sum().item() / positive_total
        else:
            positive_accuracy = -1

        negative_mask = (delta == 0).int()
        negative_total = negative_mask.sum()
        if negative_total > 0:
            negative_accuracy = ((delta_pred == delta).int() * negative_mask).sum().item() / negative_total
        else:
            negative_accuracy = -1
        ...
        self.log('val_neg_acc', negative_accuracy, on_step=True, on_epoch=False, prog_bar=True, logger=True, sync_dist=True)
        self.log('val_pos_acc', positive_accuracy, on_step=True, on_epoch=False, prog_bar=True, logger=True, sync_dist=True)

        # accuracy = (delta_pred == delta).sum().item() / delta.shape[0]
    ...
    def load_weights(self, checkpoint_path):
        ...
        for layer in self.delta_linear:
        ...
        for name, param in self.named_parameters():
            param.requires_grad = False
            if 'delta_linear' in name or 'cross_graph_att_layers' in name or 'mim_layers' in name or 'diff_layer' in name: # or 'cross_graph_att_layers' in name or 'mim_layers' in name or 'diff_layer' in name
...
def main():
    ...
    train_dataset = load_from_disk('/home/tc415/muPPIt/dataset/train/delta_correct_2')
    val_dataset = load_from_disk('/home/tc415/muPPIt/dataset/val/delta_correct_2')
    # val_dataset = None
    tokenizer = AutoTokenizer.from_pretrained("facebook/esm2_t33_650M_UR50D")
    ...
    logger = WandbLogger(project=f"muppit",
                        #  name="debug",
                         name=f"delta3_lr={args.lr}_gradclip={args.grad_clip}",
